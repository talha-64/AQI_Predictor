...
def main():
    ...
    # 2. Fetch latest features from Hopsworks
    logger.info(f"Fetching latest feature records from '{FEATURE_GROUP_NAME}_v{FEATURE_GROUP_VERSION}'...")
    fg = feature_store.get_feature_group(name=FEATURE_GROUP_NAME, version=FEATURE_GROUP_VERSION)

    try:
        # Primary Attempt: Read via Online Storage API (uses HTTPS REST port 443, bypassing gRPC port 5005)
        logger.info("Attempting online feature group read...")
        df_features = fg.read(online=True)
    except Exception as e:
        logger.warning(f"Online read fallback triggered ({e}). Trying offline read with explicit Arrow Flight bypass...")
        # Secondary Attempt: Offline read using explicit pandas conversion & flight bypass
        df_features = fg.select_all().read(
    ...
    # Ensure timestamp data type matches and sort chronologically
    if "timestamp_unix" in df_features.columns:
        df_features["timestamp_unix"] = pd.to_numeric(df_features["timestamp_unix"])
        df_features = df_features.sort_values("timestamp_unix").tail(200).reset_index(drop=True)
    else:
        df_features = df_features.tail(200).reset_index(drop=True)
